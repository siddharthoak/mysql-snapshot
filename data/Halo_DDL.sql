...
CREATE TABLE `accounts` (
  ...
  `account_type` enum('savings','checking','loan','investment','credit_card','mortgage','retirement','other','depository') NOT NULL,
  `account_origin` enum('MANUAL','AUTO','SYSTEM') NOT NULL DEFAULT 'MANUAL',
  `current_value` decimal(15,2) NOT NULL DEFAULT '0.00',
  `equity` decimal(15,2) DEFAULT NULL COMMENT 'Equity value for mortgage, loan accounts',
  ...
  KEY `idx_account_user_type` (`user_id`,`account_type`),
  ...
  KEY `ix_accounts_account_origin` (`account_origin`),
  ...
  CONSTRAINT `ck_accounts_equity_only_for_liability` CHECK (((`equity` is null) or (`account_type` in (_utf8mb4'mortgage',_utf8mb4'loan'))))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `chat_messages` (
  ...
  `crew_unclass_guard` smallint GENERATED ALWAYS AS ((case when ((`crew_type` is null) or (`crew_type` = _utf8mb4'UnclassifiedCrew')) then 1 else NULL end)) STORED,
  `chat_session_id` binary(16) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_chat_messages_trace_unclass_guard` (`trace_id`,`crew_unclass_guard`),
  KEY `idx_chat_messages_trace_crew_created` (`trace_id`,`crew_type`,`created_at`,`id`),
  KEY `ix_chat_messages_chat_session_id` (`chat_session_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE TABLE `chat_summary` (
  `id` binary(16) NOT NULL COMMENT 'Unique identifier for the chat summary record',
  `customer_id` binary(16) NOT NULL COMMENT 'Unique identifier for the customer',
  `session_id` binary(16) NOT NULL COMMENT 'Unique identifier for the session',
  `trace_id` binary(16) NOT NULL COMMENT 'Unique identifier for tracing the request',
  `summary_response` json DEFAULT NULL COMMENT 'JSON response containing the chat summary',
  `start_date_time` datetime DEFAULT NULL COMMENT 'Start datetime of the conversation window',
  `end_date_time` datetime DEFAULT NULL COMMENT 'End datetime of the conversation window',
  `client_timestamp` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Timestamp when the client made the request',
  `agent_timestamp` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Timestamp when the agent generated the response',
  `status` enum('pending','completed','failed') NOT NULL DEFAULT 'pending' COMMENT 'Status of the chat summary generation',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `ix_chat_summary_trace_id` (`trace_id`),
  KEY `ix_chat_summary_customer_id` (`customer_id`),
  KEY `ix_chat_summary_created_at` (`created_at`),
  KEY `ix_chat_summary_customer_id_created_at` (`customer_id`,`created_at`),
  KEY `ix_chat_summary_status` (`status`),
  CONSTRAINT `fk_chat_summary_customer_id_customer` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Stores chat conversation summaries for customers';
...
CREATE TABLE `customer_access_tokens` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `idx_customer_access_token_item` (`item_id`),
  KEY `idx_customer_access_token_customer` (`customer_id`),
  KEY `idx_customer_access_token_last_used` (`last_used_at`),
  CONSTRAINT `fk_customer_access_tokens_customer_id` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `growth_projections` (
  ...
  `created_at` datetime DEFAULT (now()),
  `updated_at` datetime DEFAULT (now()),
  `narrated_info` json DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `networth` (
  `id` binary(16) NOT NULL,
  `customer_id` binary(16) NOT NULL,
  `depository_accounts` decimal(15,2) NOT NULL DEFAULT '0.00',
  `investment_account` decimal(15,2) NOT NULL DEFAULT '0.00',
  `retirement_account` decimal(15,2) NOT NULL DEFAULT '0.00',
  `other_assets` decimal(15,2) NOT NULL DEFAULT '0.00',
  `credit_card` decimal(15,2) NOT NULL DEFAULT '0.00',
  `student_loan` decimal(15,2) NOT NULL DEFAULT '0.00',
  `mortgage` decimal(15,2) NOT NULL DEFAULT '0.00',
  `other_liabilities` decimal(15,2) NOT NULL DEFAULT '0.00',
  `currency` enum('USD','EUR','GBP') DEFAULT NULL,
  `narrated_info` json DEFAULT NULL,
  `created_at` datetime DEFAULT NULL,
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_networth_customer_id` (`customer_id`),
  KEY `ix_networth_customer_id` (`customer_id`),
  CONSTRAINT `fk_networth_customer_id` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;


CREATE TABLE `money_flow_analytics` (
  `id` binary(16) NOT NULL,
  `customer_id` binary(16) NOT NULL,
  `period_type` varchar(20) NOT NULL,
  ...
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `ix_money_flow_analytics_customer_id` (`customer_id`),
  CONSTRAINT `fk_money_flow_analytics_customer_id` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE `plaid_sync_cursors` (
  ...
  `updated_at` datetime NOT NULL DEFAULT (now()),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_customer_item_cursor` (`customer_id`,`item_id`),
  KEY `ix_plaid_sync_cursors_customer_id` (`customer_id`),
  KEY `ix_plaid_sync_cursors_item_id` (`item_id`),
  KEY `ix_plaid_sync_cursors_customer_item` (`customer_id`,`item_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
